...
loglikelihoodAcc (A.T2 dataX dataY) (A.T2 (A.the -> bias) theta) =
  let Z :. rows :. _ = A.unlift (A.shape dataX) :: Z :. A.Exp Int :. A.Exp Int
      -- broadTheta = A.backpermute (A.shape dataX) (A.index1 . A.indexHead) theta
      broadTheta = A.replicate (A.lift (Z :. rows :. A.All)) theta
