...
translateAccInPVal vt pv prf (A.OpenAcc expr) = case expr of
    ...
    _ -> internalError (fromString "AD.translateAccInPVal: Cannot perform AD on Acc node <" % fmap ($ expr) A.formatPreAccOp % fromString ">")
  where
...
translateExpInPVal vt avt pv prf expr = case expr of
    ...
    _ -> internalError (fromString "AD.translateExp: Cannot perform AD on Exp node <" % fmap ($ expr) A.formatExpOp % fromString ">")
...
untranslateLHSboundExp toplhs topexpr topaweak topweak
  ...
  where
    ...
    go aw w pv expr = case expr of
        ...
        D.Shape _ (D.ARLab _) -> internalError (fromString "AD.untranslateLHSboundExp: Cannot translate label (Shape) in array var position")
        D.Index _ (D.ARVar avar) _ e -> A.Index avar (go aw w pv e)
        D.Index _ (D.ARFree avar) _ e -> A.Index (A.weaken aw avar) (go aw w pv e)
        D.Index _ (D.ARLab _) _ _ -> internalError (fromString "AD.untranslateLHSboundExp: Cannot translate label (Index) in array var position")
        ...
        D.Arg _ _ _ -> internalError (fromString "AD.untranslateLHSboundExp: Unexpected Arg in untranslate!")
...
untranslateLHSboundExpA toplhs topexpr arrweak arrpv
  ...
  where
    ...
    go aw pv expr = case expr of
        ...
        -- TODO: Don't use a fallible call here, perhaps something with the double-tracking of PartialVal' ?
        D.Var _ var _ -> A.Evar (fromJust (checkLocalPFallible matchScalarType var pv))
        D.FreeVar _ _ -> internalError (fromString "AD.untranslateLHSboundExpA: Unexpected free expression variable in array code")
        D.Let lhs def body
          | A.Exists lhs' <- A.rebuildLHS lhs
        ...
        D.Shape _ (D.ARFree avar) -> A.Shape (A.weaken aw avar)
        D.Shape _ (D.ARLab _) -> internalError (fromString "AD.untranslateLHSboundExpA: Cannot translate label (Shape) in array var position")
        ...
        D.Index _ (D.ARLab _) _ _ -> internalError (fromString "AD.untranslateLHSboundExpA: Cannot translate label (Index) in array var position")
        D.ShapeSize _ sht e -> A.ShapeSize sht (go aw pv e)
        D.Get _ path e
          | D.LetBoundVars lhs vars <- euntranslateGet (D.etypeOf e) path
          -> A.Let lhs (go aw pv e) (a_evars vars)
        D.Undef lab -> A.Undef (labelType lab)
        D.Arg _ _ _ -> internalError (fromString "AD.untranslateLHSboundExpA: Unexpected Arg in untranslate!")
...
untranslateLHSboundAcc toplhs topexpr topweak
  ...
  where
    ...
    go w pv expr = A.OpenAcc $ case expr of
        ...
        D.Aarg _ _ _ -> internalError (fromString "AD.untranslateLHSboundAcc: Unexpected Arg in untranslate!")
